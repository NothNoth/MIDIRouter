...
func New(name string, channel filter.FilterChannel, settings json.RawMessage) (*GenNoteOff, error) {
	var g GenNoteOff
...
func (g *GenNoteOff) Generate(packet coremidi.Packet, value uint16) (generate coremidi.Packet, err error) {
	var statusByte byte
	var note byte
	var velocity byte
	...
	if g.channel == filter.FilterChannelAny {
		statusByte = byte(filter.FilterMsgTypeNoteOff<<4 | filteredChannel)
	} else {
		statusByte = byte(filter.FilterMsgTypeNoteOff<<4 | g.channel)
	}
...
func (g *GenNoteOff) String() string {
	str := fmt.Sprintf("NoteOff (channel %s) ", g.channel.String())
