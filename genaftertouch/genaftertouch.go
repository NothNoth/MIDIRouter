...
func (g *GenAftertouch) Generate(packet coremidi.Packet, value uint16) (generate coremidi.Packet, err error) {
	var statusByte byte
	var pressure byte
	...
	if g.channel == filter.FilterChannelAny {
		statusByte = byte(filter.FilterMsgTypeAftertouch<<4 | filteredChannel)
	} else {
		statusByte = byte(filter.FilterMsgTypeAftertouch<<4 | g.channel)
	}
...
func (g *GenAftertouch) String() string {
	str := fmt.Sprintf("Aftertouch (channel %s) ", g.channel.String())
